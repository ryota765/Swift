...
class RecommendViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, TimelineTableViewCellDelegate {
    ...
    override func viewDidLoad() {
        ...
        //view.backgroundColor = self.black
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //cell.userImageButton.imageView?.kf.setImage(with: URL(string: userImageUrl), placeholder: UIImage(named: "placeholder.jpg")
    ...
    func didTapSaveButton(tableViewCell: UITableViewCell, button: UIButton) {
        ...
        if posts[tableViewCell.tag].isSaved == false || posts[tableViewCell.tag].isSaved == nil{
            let query = NCMBQuery(className: "Post")
            query?.getObjectInBackground(withId: posts[tableViewCell.tag].objectId, block: { (post, error) in
                post?.addUniqueObject(currentUser.objectId, forKey: "saveUser")
                post?.saveEventually({ (error) in
                    if error != nil{
                        SVProgressHUD.showError(withStatus: error!.localizedDescription)
                    }else{
                ...
                for postObject in result as! [NCMBObject]{
                    ...
                    if user.object(forKey: "active") as? Bool != false{
                        ...
                        file.getDataInBackground { (data, error) in
                            if error != nil{
                                //self.images.append(UIImage(named: "placeholder.jpg"))
                                SVProgressHUD.showError(withStatus: error!.localizedDescription)
                            }else{
                                if data != nil{
                                    let image = UIImage(data: data!)
                                    self.images.append(image ?? UIImage(named: "placeholder.jpg")!)
                                }else {
                                    self.images.append(UIImage(named: "placeholder.jpg")!)
                                }
                            }
